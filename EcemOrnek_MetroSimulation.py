...
from heapq import heappop, heappush  # A* algoritması için
...
class MetroAgi:
    ...
    def en_az_aktarma_bul(self, baslangic_id: str, hedef_id: str) -> Optional[List[Istasyon]]:
        ...
        if baslangic_id not in self.istasyonlar or hedef_id not in self.istasyonlar:
            return None  # Başlangıç veya hedef istasyon yoksa None döndürür.

        queue = deque([(self.istasyonlar[baslangic_id], [self.istasyonlar[baslangic_id]])])
        ...
        while queue:
            ...
            if mevcut_istasyon.idx == hedef_id:
                return yol  # Hedefe ulaşılırsa yolu döndürür.
    ...
    def en_hizli_rota_bul(self, baslangic_id: str, hedef_id: str) -> Optional[Tuple[List[Istasyon], int]]:
        ...
        pq = [(0, id(self.istasyonlar[baslangic_id]), self.istasyonlar[baslangic_id], [self.istasyonlar[baslangic_id]])]
        ziyaret_edilenler = {}

        while pq:
            toplam_sure, _, mevcut_istasyon, yol = heappop(pq)

            if mevcut_istasyon.idx == hedef_id:  # Hedefe ulaşıldıysa rn hızlı rotayı döndürür
                return yol, toplam_sure

            # Eğer daha kısa süresede ulaşılmışsa bu yoldan devam et
            if mevcut_istasyon.idx in ziyaret_edilenler and ziyaret_edilenler[mevcut_istasyon.idx] <= toplam_sure:
                continue

            ziyaret_edilenler[mevcut_istasyon.idx] = toplam_sure

            for komsu, gecis_suresi in mevcut_istasyon.komsular:
                yeni_sure = toplam_sure + gecis_suresi
                heappush(pq, (yeni_sure, id(komsu), komsu, yol + [komsu]))
